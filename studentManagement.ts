...
class Student {
  ...
  enrollInCourses(selectedCourses: string[]) {
  ...
  while (true) {
    ...
    switch (choice.action) {
      ...
      case 'Enroll in Courses':
        ...
        if (studentToEnroll) {
          const selectedCourses: { courses: string[] } = await inquirer.prompt({
            type: 'checkbox',
            name: 'courses',
            message: 'Select courses to enroll:',
            choices: ['Math', 'English', 'Science'],
      ...
      case 'Pay Tuition Fees':
        ...
        if (studentToPayFees) {
          const tuitionAmount: { amount: number } = await inquirer.prompt({
            type: 'number',
            name: 'amount',
